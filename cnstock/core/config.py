...
class DataConfig:
    ...
    #: 单票实时行情快路径（**默认关闭**，需联网后由用户手动开启）。
    #:
    #: 开启后，``realtime`` 对自选股（通常个位数）逐只调用东财
    #: ``stock_bid_ask_em``（每只是 1 个 HTTP 请求），**不再拉全市场快照**
    #: （``stock_zh_a_spot_em`` 一次约 59 个分页请求）。仅当自选股数量远小于
    #: 全市场（约 5900 只）时才划算；若订阅了全市场请勿开启。
    #:
    #: **安全保证**：provider 内任意异常或空返回都会自动回退到 ``realtime()``
    #: （全市场快照），因此开启它绝不会比现状更差。因离线环境无法验证真实字段
    #: 映射，故默认关闭，由用户在联网环境确认无误后再开启。
    realtime_per_symbol: bool = False  # 单票实时快路径（默认关闭，安全可回退）
